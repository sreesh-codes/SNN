...
class SNNDatasetGenerator:
    ...
    def rate_encoding(self, values, max_time_steps=100, max_rate=100):
        ...
        for i in range(n_samples):
            for j in range(n_features):
                ...
                # Generate spikes
                spike_trains[i, j, :] = np.random.binomial(1, prob, max_time_steps)
    ...
    def generate_temporal_pattern_dataset(self, n_samples=1000, n_features=10,
        ...
        # Shuffle to randomize order (seed already set in __init__)
        ...
        for i in range(n_samples):
            ...
            if class_id == 0:
                # Early burst pattern
                spike_trains[i, :, :10] = np.random.binomial(1, 0.8, (n_features, 10))
            elif class_id == 1:
                # Late burst pattern
                spike_trains[i, :, -10:] = np.random.binomial(1, 0.8, (n_features, 10))
            elif class_id == 2:
                ...
                for t in range(0, pattern_length, 10):
                    spike_trains[i, :, t:t+3] = np.random.binomial(1, 0.7, (n_features, 3))
            elif class_id == 3:
                # Random sparse pattern
                spike_trains[i, :, :] = np.random.binomial(1, 0.1, (n_features, pattern_length))
            else:
                # Dense uniform pattern
                spike_trains[i, :, :] = np.random.binomial(1, 0.4, (n_features, pattern_length))
